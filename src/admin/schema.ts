import {
  ...
  FAQS, GLOBAL_CONTENT, HOME_CONTENT, NAVIGATION_ITEMS, PORTFOLIO_CONTENT,
  PORTFOLIO_ITEMS, PRODUCTION_TEAM, SERVICES_CONTENT, STATS, STUDIO_INFO, TESTIMONIAL,
} from '../data/content';

export type FieldType = 'text' | 'textarea' | 'url' | 'image' | 'toggle' | 'number' | 'list' | 'categoryList' | 'portfolioSelect' | 'objectList' | 'select';
...
const seoSection = (): SectionSchema => ({
  key: 'seo', title: 'Search Appearance', description: 'Control how this page appears in search engines.',
  fields: [
    { path: 'seo.metaTitle', label: 'SEO Title', help: 'Aim for 50–60 characters.' },
    { path: 'seo.metaDescription', label: 'Meta Description', type: 'textarea', help: 'A clear summary of around 150–160 characters.' },
    { path: 'seo.indexable', label: 'Allow search engines to show this page', type: 'toggle' },
  ],
});
...
export const CMS_SCHEMAS: Record<string, DocumentSchema> = {
  home: {
    title: 'Home Page', route: '/', description: 'Manage every content section shown on the homepage.', sections: [
      ...
      { key: 'about', title: 'About Preview', description: 'Short studio introduction displayed on the homepage.', visibilityPath: 'content.sectionVisibility.about', fields: [
        { path: 'content.aboutEyebrow', label: 'Small Label' }, { path: 'content.aboutHeading', label: 'Heading', type: 'textarea' }, { path: 'content.aboutButtonText', label: 'Button Text' }, { path: 'content.aboutToolsText', label: 'Tools Label' },
      ]},
      { key: 'stats', title: 'Statistics', description: 'Shared proof points and studio achievements.', visibilityPath: 'content.sectionVisibility.stats' },
      { key: 'services', title: 'Services Preview', description: 'Heading and link above the featured services.', visibilityPath: 'content.sectionVisibility.services', fields: [
        { path: 'content.servicesEyebrow', label: 'Small Label' }, { path: 'content.servicesHeading', label: 'Heading', type: 'textarea' }, { path: 'content.servicesCtaText', label: 'Button Text' },
      ]},
  ...
  about: {
    title: 'About Page', route: '/about', description: 'Manage the studio story, principles, process and team.', sections: [
      ...
      { key: 'process', title: 'Creative Process', description: 'Ordered workflow shown to prospective clients.', visibilityPath: 'content.sectionVisibility.process', fields: [{ path: 'content.processEyebrow', label: 'Small Label' }, { path: 'content.processHeading', label: 'Heading' }] },
      { key: 'locations', title: 'Studio Locations', description: 'Labels and descriptions for the UK and international contact cards.', visibilityPath: 'content.sectionVisibility.locations', fields: [{ path: 'content.locationPrimaryLabel', label: 'UK Small Label' }, { path: 'content.locationPrimaryHeading', label: 'UK Heading' }, { path: 'content.locationSecondaryLabel', label: 'International Small Label' }, { path: 'content.locationSecondaryHeading', label: 'International Heading' }, { path: 'content.locationSecondaryDescription', label: 'International Description', type: 'textarea' }] },
      { key: 'team', title: 'Production Team', description: 'Introduction for the shared team collection.', visibilityPath: 'content.sectionVisibility.team', fields: [{ path: 'content.teamEyebrow', label: 'Small Label' }, { path: 'content.teamHeading', label: 'Heading' }, { path: 'content.teamDescription', label: 'Description', type: 'textarea' }] },
      { key: 'finalCta', title: 'Final Call to Action', description: 'Closing heading and buttons.', visibilityPath: 'content.sectionVisibility.finalCta', fields: [{ path: 'content.finalHeading', label: 'Heading' }, { path: 'content.finalPrimaryText', label: 'Primary Button Text' }, { path: 'content.finalSecondaryText', label: 'Secondary Button Text' }] }, seoSection(),
    ],
  },
  services: {
    title: 'Services', route: '/services', description: 'Manage the services landing page and every service detail.', sections: [
      ...
      { key: 'items', title: 'Service Cards', description: 'Services are managed once in Global Content. Use this section for the Services page display setting.', visibilityPath: 'content.sectionVisibility.list' },
  ...
  contact: {
    title: 'Contact Page', route: '/contact', description: 'Manage contact copy, form language, response promise and FAQs.', sections: [
      ...
      { key: 'faqs', title: 'Frequently Asked Questions', description: 'FAQs are managed once in Global Content. Use this section for the Contact page display setting.', visibilityPath: 'content.sectionVisibility.faqs', fields: [{ path: 'content.faqEyebrow', label: 'Small Label' }, { path: 'content.faqHeading', label: 'Heading' }] }, seoSection(),
    ],
  },
  global: {
    title: 'Header & Footer', route: '/', description: 'Manage site identity, navigation, business information and footer copy.', sections: [
      ...
      { key: 'navigation', title: 'Header Navigation', description: 'Menu labels, destinations, visibility and order.', repeater: { path: 'navigation', itemName: 'Menu Item', titlePath: 'label', fields: [{ path: 'label', label: 'Menu Label' }, { path: 'url', label: 'Destination', type: 'url' }, { path: 'visible', label: 'Show Link', type: 'toggle' }] } },
      ...
      { key: 'header', title: 'Header Labels', description: 'Service-menu and mobile-header wording.', fields: [{ path: 'content.servicesMenuLabel', label: 'Services Menu Label' }, { path: 'content.servicesMenuAllText', label: 'View All Services Text' }, { path: 'content.startProjectText', label: 'Main Button Text' }, { path: 'content.mobilePhoneLabel', label: 'Mobile Phone Label' }] },
...
export function hydrateCmsDocument(key: string, source: Record<string, unknown>): Record<string, unknown> {
  ...
  if (key === 'about') return { ...value, content: hydrateAboutContent(value.content), process: Array.isArray(value.process) && value.process.length ? value.process : clone(CREATIVE_PROCESS), team: Array.isArray(value.team) && value.team.length ? value.team : clone(PRODUCTION_TEAM) };
  ...
  if (key === 'portfolio') return { ...value, content: merge(PORTFOLIO_CONTENT, value.content), items: (Array.isArray(value.items) && value.items.length ? value.items : clone(PORTFOLIO_ITEMS)).map((item: Record<string, unknown>) => ({ visible: true, ...item })) };
