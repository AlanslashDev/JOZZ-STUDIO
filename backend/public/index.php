...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && preg_match('#/uploads/([a-f0-9]{32}\.(?:jpg|png|webp|gif|mp4|webm))$#', $path, $mediaMatch)) {
    $file = BASE_PATH . '/storage/uploads/' . $mediaMatch[1];
...
if ($method === 'GET' && $route === '/admin/auth/me') {
    ...
    if (!is_array($admin)) Http::error('Authentication required.', 401);
    Http::ok(['admin' => $admin, 'csrfToken' => $_SESSION['csrf']]);
}
...
if ($method === 'DELETE' && preg_match('#^/admin/content/(services|portfolio|faqs|categories|redirects|navigation)/(\d+)$#',$route,$m)) { $admin=Auth::requireAdmin(); $tables=['services'=>'services','portfolio'=>'portfolio_items','faqs'=>'faqs','categories'=>'portfolio_categories','redirects'=>'redirects','navigation'=>'navigation_items'];$s=db()->prepare('DELETE FROM '.$tables[$m[1]].' WHERE id=:id');$s->execute(['id'=>(int)$m[2]]);if(!$s->rowCount())Http::error('Item not found.',404);adminAudit($admin,'deleted',$m[1],$m[2]);clearPublicContent();Http::ok(['deleted'=>true]); }
if ($method === 'POST' && $route === '/admin/media') { $admin=Auth::requireAdmin(); if(empty($_FILES['file'])||$_FILES['file']['error']!==UPLOAD_ERR_OK)Http::error('Choose a file to upload.',422);$f=$_FILES['file'];if($f['size']>25*1024*1024)Http::error('Files must be smaller than 25 MB.',422);$mime=(new finfo(FILEINFO_MIME_TYPE))->file($f['tmp_name']);$allowed=['image/jpeg','image/png','image/webp','image/gif','video/mp4','video/webm'];if(!in_array($mime,$allowed,true))Http::error('Upload a JPG, PNG, WebP, GIF, MP4, or WebM file.',422);$ext=['image/jpeg'=>'jpg','image/png'=>'png','image/webp'=>'webp','image/gif'=>'gif','video/mp4'=>'mp4','video/webm'=>'webm'][$mime];$name=bin2hex(random_bytes(16)).'.'.$ext;$dir=BASE_PATH.'/storage/uploads';if(!is_dir($dir))mkdir($dir,0775,true);if(!move_uploaded_file($f['tmp_name'],$dir.'/'.$name))Http::error('Upload could not be stored.',500);$path='/uploads/'.$name;$s=db()->prepare('INSERT INTO media(path,original_name,mime_type,size_bytes,alt_text,created_by) VALUES(:p,:n,:m,:z,:a,:u)');$s->execute(['p'=>$path,'n'=>mb_substr($f['name'],0,255),'m'=>$mime,'z'=>$f['size'],'a'=>mb_substr(trim((string)($_POST['altText']??'')),0,255),'u'=>$admin['id']]);adminAudit($admin,'uploaded','media',(string)db()->lastInsertId());Http::ok(['id'=>(int)db()->lastInsertId(),'url'=>$path],201); }
if ($method === 'GET' && $route === '/admin/media') { Auth::requireAdmin();Http::ok(['items'=>db()->query('SELECT id,path AS url,original_name AS name,mime_type AS type,size_bytes AS size,alt_text AS altText,created_at AS createdAt FROM media ORDER BY id DESC')->fetchAll()]); }
if ($method === 'GET' && $route === '/admin/enquiries/export') { Auth::requireAdmin(); header('Content-Type: text/csv; charset=utf-8');header('Content-Disposition: attachment; filename="joozz-enquiries.csv"');$o=fopen('php://output','w');fputcsv($o,['Name','Email','Phone','Service','Status','Received']);foreach(db()->query('SELECT name,email,phone,service,status,created_at FROM enquiries ORDER BY created_at DESC') as $r)fputcsv($o,$r);fclose($o);exit; }
